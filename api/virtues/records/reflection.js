...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (reflection && reflection.length > 1000) {
      return res.status(400).json({ message: '每日反思不能超过1000个字符' });
    }
    ...
    if (recordData) {
      ...
      if (recordData.dailyReflection && typeof recordData.dailyReflection === 'object') {
        console.warn('Found corrupted dailyReflection data in reflection.js, fixing:', recordData.dailyReflection);
      ...
      if (recordData.virtues && Array.isArray(recordData.virtues)) {
        console.warn('Found corrupted virtues data (array format) in reflection.js, converting to Map:', recordData.virtues);
        const virtuesMap = new Map();
    ...
    if (reflection !== undefined) {
      record.dailyReflection = reflection;
    }
    if (rating !== undefined) {
      record.dailyRating = rating;
    }
    ...
    res.status(200).json({
      ...
      record: {
        date: record.date,
        dailyReflection: record.dailyReflection,
        dailyRating: record.dailyRating
      }
    });
